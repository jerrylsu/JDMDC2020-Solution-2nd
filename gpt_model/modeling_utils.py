...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        Parameters:
            pretrained_model_name_or_path: either:
              - a string with the `shortcut name` of a pre-trained model to load from cache_no_pretrained or download, e.g.: ``bert-base-uncased``.
            ...
            cache_dir: (`optional`) string:
                ...
                configuration should be cached if the standard cache_no_pretrained should not be used.
        ...
        Examples::
            ...
            model = BertModel.from_pretrained('bert-base-uncased')    # Download model and configuration from S3 and cache_no_pretrained.
        ...
        if pretrained_model_name_or_path is not None:
            ...
            # redirect to the cache_no_pretrained, if necessary
            try:
            ...
            if resolved_archive_file == archive_file:
                ...
                logger.info("loading weights file {} from cache_no_pretrained at {}".format(archive_file, resolved_archive_file))
        else:
        ...
        if from_tf:
            ...
            for key in state_dict.keys():
                ...
                if "tokens_embed" in key:
                    new_key = key.replace("tokens_embed", "wte")
                if "positions_embed" in key:
                    new_key = key.replace("positions_embed", "wpe")
                if new_key:
                    old_keys.append(key)
    ...
    def generate(
        ...
        Examples::
            ...
            model = AutoModelWithLMHead.from_pretrained('distilgpt2')    # Download model and configuration from S3 and cache_no_pretrained.
            ...
            model = AutoModelWithLMHead.from_pretrained('openai-gpt')    # Download model and configuration from S3 and cache_no_pretrained.
            ...
            model = AutoModelWithLMHead.from_pretrained('ctrl')    # Download model and configuration from S3 and cache_no_pretrained.
    ...
    def _generate_beam_search(
        ...
        token_type_ids = token_type_ids.unsqueeze(1).expand(batch_size, num_beams, cur_len)
        token_type_ids = token_type_ids.contiguous().view(batch_size * num_beams, cur_len)  # (batch_size * num_beams, cur_len)
        ...
        # cache_no_pretrained compute states
        past = None
        ...
        while cur_len < max_length:
            model_inputs = self.prepare_inputs_for_generation(input_ids, token_type_ids=token_type_ids, input_images=input_images, image_ids=image_ids, past=past)
