...
class PreTrainedTokenizer(object):
    ...
    def from_pretrained(cls, *inputs, **kwargs):
        ...
        Args:
            pretrained_model_name_or_path: either:
                ...
                - a string with the `shortcut name` of a predefined tokenizer to load from cache_no_pretrained or download, e.g.: ``bert-base-uncased``.
                - a string with the `identifier name` of a predefined tokenizer that was user-uploaded to our S3, e.g.: ``dbmdz/bert-base-german-cased``.
                - a path to a `directory` containing vocabulary files required by the tokenizer, for instance saved using the :func:`~transformers.PreTrainedTokenizer.save_pretrained` method, e.g.: ``./my_model_directory/``.
                - (not applicable to all derived classes, deprecated) a path or url to a single saved vocabulary file if and only if the tokenizer only requires a single vocabulary file (e.g. Bert, XLNet), e.g.: ``./my_model_directory/vocab.txt``.

            cache_dir: (`optional`) string:
                Path to a directory in which a downloaded predefined tokenizer vocabulary files should be cached if the standard cache_no_pretrained should not be used.
        ...
        Examples::
            ...
            # Download vocabulary from S3 and cache_no_pretrained.
            tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')

            # Download vocabulary from S3 (user-uploaded) and cache_no_pretrained.
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        # Get files from url, cache_no_pretrained, or disk depending on the case
        try:
        ...
        for file_id, file_path in vocab_files.items():
            if file_path == resolved_vocab_files[file_id]:
                ...
                logger.info("loading file {} from cache_no_pretrained at {}".format(file_path, resolved_vocab_files[file_id]))
